...
codename='void'
#codename=$(awk -F '=' '/^ID=/ {print $NF;}' /etc/os-release)

case $codename in
[vV]oid)
cat <<EOF
$PADCC$PBLD                ${PGRN}________
$PADCC              _ \______ -
$PADCC             | \\  ${PWHT}___ ${PGRN} \\ |
$PADCC             | | ${PWHT}/   \\ ${PGRN}| |
$PADCC             | | ${PWHT}\\___/ ${PGRN}| |
...
$PADCC             ooooo   ooooo
$PADCC            ooooo     ooooo
$PADCC           ${PBLU}ooooo  ${PWHT}<oooooooo>
$PADCC          ${PBLU}ooooo      ${PWHT}<oooooo>
$PADCC         ${PBLU}ooooo          ${PWHT}<oooo>
EOF
;;
[mM]anjaro)
cat <<EOF
...
NOUTT="$BLK$NBAR$RED$NBAR$GRN$NBAR$YLW$NBAR$BLU$NBAR$PUR$NBAR$CYN$NBAR$WHT$NBAR$RST"
#
printf "%s%b" "$PADC" "$NOUTT"
printf "\n"
...
# environment
printf "%s%b" "$PADC" "$RED        distro $WHT$SEP $RED$distro\n"
printf "%s%b" "$PADC" "$BLU        kernel $WHT$SEP $BLU$(uname -r)\n"
printf "%s%b" "$PADC" "$PUR      packages $WHT$SEP $PUR$(get_pkg_count)\n"
printf "%s%b" "$PADC" "$GRN         shell $WHT$SEP $GRN${SHELL##*/}\n"
printf "%s%b" "$PADC" "$YLW          term $WHT$SEP $YLW$term\n"
printf "%s%b" "$PADC" "$CYN            wm $WHT$SEP $CYN${wm}\n"
printf "%s%b" "$PADC" "$WHT          disk $WHT$SEP $WHT$storage used\n"
printf " $RST\n"

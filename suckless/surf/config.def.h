...
/* URL Bar */
#define URLPROP(r, s, p) { \
        .v = (const char *[]){ "/bin/sh", "-c", \
             "prop=\"$(printf '%b' \"$(xprop -id $1 "r" " \
             "| sed -e 's/^"r"(UTF8_STRING) = \"\\(.*\\)\"/\\1/' " \
             "      -e 's/\\\\\\(.\\)/\\1/g' " \
             "&& tac ~/.config/surf/bookmarks | cat -n | sed 's/\\t/. /g' " \
             "| sed 's/     / /')\" " \
             "| dmenu -l 10 -p '"p"' -w $1 | awk '{print $NF;}')\" " \
             "&& xprop -id $1 -f "s" 8u -set "s" \"$prop\"", \
             "surf-setprop", winid, NULL \
        } \
}
...
#define DOWNLOAD(d, r) { \
        .v = (char *[]){ "/bin/sh", "-c", \
                "cd ~/dl/surf/;"\
                "st -t Terminal -e /bin/sh -c \"aria2c -m 0 -c -x 16 -U '$1'" \
...
#define BM_ADD(r) {\
        .v = (const char *[]){ "/bin/sh", "-c", \
             ...
             "cp ~/.config/surf/bookmarks.tmp ~/.config/surf/bookmarks" \
             "&& notify-send -a 'Bookmark added' `cat ~/.config/surf/bookmarks.tmp | head -1` " \
             "&& rm ~/.config/surf/bookmarks.tmp", \
             winid, r, NULL \
        } \
}
...
static Key keys[] = {
        ...
        { 0,                GDK_KEY_g,      spawn,      URLPROP("_SURF_URI", "_SURF_GO", PROMPT_GO) },
        { 0,                GDK_KEY_b,      spawn,      URLPROP("_SURF_URI", "_SURF_GO", PROMPT_GO) },
